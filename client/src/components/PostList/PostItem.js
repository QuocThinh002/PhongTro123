...
const PostItem = (props) => {
    ...
    return (
        <div className={` ${star === 5 ? 'bg-outstanding' : 'bg-white'}  border-t-[1px]  ${levelStar[star].borderColor} bg-orange-50 flex items-center  gap-4 p-3`}>
            <div className={`${star === 5 ? 'w-2/5' : 'w-[28%]'} aspect-[4/3] relative `}>
                ...
                <span onClick={handleLike} className={`absolute bottom-1 right-2 cursor-pointer ${isLiked ? 'text-red-500':'text-white'}`}><FaHeart size={20} /></span>
            </div>
            <div className='flex-1'>
                <div className=' line-clamp-2'>
                    <span>
                        ...
                        <span className={`${levelStar[star].textColor} ${star >= 2 ? 'uppercase' : 'normal-case'} ${star ? 'ml-2 text-lg font-bold' : 'font-medium'} `}>{post?.title}</span>
                        </Link>
                    </span>
